...
- То же самое подтверждение можно сделать **прямо из Telegram**, не
  открывая приложение: уведомление о новой записи, которое приходит в
  `TELEGRAM_ADMIN_CHAT_ID` (`lib/notify.js`), несёт inline-кнопку
  «✅ Подтвердить». Нажатие обрабатывается в
  `pages/api/bot/telegram-webhook.js` (`callback_query` с
  `data: "confirm:<id>"`), проверяет, что нажавший — id из `ADMIN_IDS`
  (`lib/auth/requireActiveUser.js#isAdminId`), и вызывает ту же функцию
  `confirmAppointment()` (`lib/appointments/confirmAppointment.js`), что и
  кнопка в приложении — клиент получает то же уведомление, а кнопка в
  чате исчезает.
