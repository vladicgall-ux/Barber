...
│   │   └── vkApi.js                  # сообщения сообщества VK + callback-кнопки
...
│       │   └── vk-webhook.js         # POST Callback API сообщества VK
│       ├── cron/
...
| `VK_GROUP_TOKEN` | VK → Управление сообществом → Работа с API → Ключи доступа (право «Сообщения») | нет |
| `VK_GROUP_ID` | id сообщества ВКонтакте | нет |
| `VK_ADMIN_ID` | ваш личный VK id — получает все уведомления в личные сообщения от сообщества | нет |
| `VK_CALLBACK_CONFIRMATION` | VK → Callback API — «Строка подтверждения» | да, для кнопок Подтвердить/Отменить в VK |
| `VK_CALLBACK_SECRET` | VK → Callback API — «Секретный ключ» (необязательно, но желательно) | нет |
...
5. **Уведомления и подтверждение записи из VK** (аналог Telegram-бота —
   подробнее в разделе 9):
   - В сообществе (группе) барбершопа: **Управление → Работа с API →
     Ключи доступа сообщества** → создайте ключ с правом **«Сообщения»**
     → `VK_GROUP_TOKEN`. ID сообщества → `VK_GROUP_ID`.
   - Впишите свой личный VK id в `VK_ADMIN_ID` — на него будут приходить
     личные сообщения от сообщества с новыми записями. **Важно:** сначала
     напишите что-нибудь этому сообществу от своего аккаунта (или откройте
     Mini App) — иначе VK не разрешит сообществу написать вам первым.
   - **Управление → Настройки → Работа с API → Callback API**: в поле
     «Adress» укажите `https://<ваш-домен>/api/bot/vk-webhook`, нажмите
     сохранить. VK покажет **«Строку подтверждения»** — скопируйте её в
     `VK_CALLBACK_CONFIRMATION`. Там же есть необязательный «Секретный
     ключ» → `VK_CALLBACK_SECRET`.
   - В типах событий Callback API включите **«Событие с клавиатуры»**
     (`message_event`) — именно так VK доставляет нажатия кнопок
...
1. `POST /api/auth/code/request` создаёт 6-значный код и случайный
   ...
   (таблица `auth_codes`). Заодно возвращает готовые ссылки на чаты —
   `telegramChatUrl` (`https://t.me/<бот>?start=<код>`, официально
   задокументированный Telegram deep link), `maxChatUrl` (тот же формат —
   предположение по аналогии, т.к. точный синтаксис MAX не подтверждён) и
   `vkChatUrl` (`https://vk.com/write-<VK_GROUP_ID>` — у VK нет диплинка с
   автоподстановкой сообщения, поэтому код туда нужно вставить вручную).
2. `components/AuthGate.js` показывает код крупно, кнопку **«Копировать»**
   и три иконки — Telegram/MAX/VK — каждая открывает свой чат по ссылке
   выше в новой вкладке.
3. Пользователь отправляет код боту/сообществу (Telegram, MAX или VK —
   любому из трёх). Открытие диплинка Telegram (`?start=<код>`)
   присылает боту `/start <код>` вместо голого кода — вебхуки это тоже
   распознают.
4. Вебхук (`pages/api/bot/telegram-webhook.js` / `max-webhook.js` /
   `vk-webhook.js` → общая логика в `lib/bot/handleIncomingMessage.js`)
   находит код, привязывает его к `telegram_id`/`max_id`/`vk_id`
   отправителя, создаёт/обновляет пользователя и помечает код как
   `claimed`. Для VK это `message_new` в Callback API — тот же вебхук,
   что уже принимает `message_event` от кнопок подтверждения записи
   (раздел 9).
5. Браузер каждые 3 секунды опрашивает `GET /api/auth/code/poll?pollToken=...`
   (`lib/useAuth.js`). Как только код помечен `claimed`, сервер выдаёт
   сессионную cookie и возвращает пользователя.

У VK нет аналога кнопки «Поделиться контактом» в обычном диалоге — для
VK-пользователей телефон подтверждается только через Mini App (канал 3),
даже если они вошли через код в диалоге с сообществом.
...
- Точно так же работает **диалог с сообществом ВКонтакте**
  (`lib/bot/vkApi.js`, `pages/api/bot/vk-webhook.js`): фиксированный
  админ (`VK_ADMIN_ID`) и админы, привязанные к мастеру
  (`notifyMasterAdmins()`), получают от сообщества личное сообщение с той
  же информацией о записи и клавиатурой из двух callback-кнопок —
  «✅ Подтвердить» / «❌ Отменить». Нажатие приходит в Callback API как
  событие `message_event`, сервер проверяет права так же через
  `isAdminByPlatformId()` (учитывает и VK id), вызывает
  `confirmAppointment()`/`cancelAppointment()`, отвечает всплывающим
  уведомлением (`messages.sendMessageEventAnswer`) и убирает клавиатуру
  из сообщения (`messages.edit`). Клиент, если входил через VK, получает
  такое же сообщение о подтверждении/отмене, как и в Telegram/MAX.
...
- Блок **«Выходные дни»** (`components/ClosedDatesManager.js`) наверху
  панели закрывает даты в таблице `closed_dates`
  (`POST/GET/DELETE /api/admin/closed-dates`):
  - **Диапазон дат** — поля «Даты» и «по» (необязательно): указав обе,
    закрываются сразу все дни диапазона одним действием (максимум 180 дней
    за раз) — каждый день сохраняется отдельной строкой, чтобы их можно
    было открыть по отдельности.
  - **Весь барбершоп или один мастер** — как и раньше, но если сам админ
    привязан к конкретному мастеру (`admin_master_id` — см. раздел
    «Пользователи» выше), выбор мастера у него скрыт и жёстко
    зафиксирован на своего: закрыть «весь барбершоп» или чужого мастера
    он не может. Это проверяется не только в интерфейсе
    (`components/ClosedDatesManager.js`), но и на сервере
    (`pages/api/admin/closed-dates.js` игнорирует присланный `masterId`
    и подставляет `admin_master_id` такого админа принудительно — так же
    и `GET`/`DELETE` видят и могут открыть только его собственные
    закрытые даты).
  - **Весь день или конкретное время** — если оставить поля времени
    пустыми, закрывается весь день (как раньше); если выбрать, например,
    08:00–10:00, блокируются только эти часы, а остальное время того же
    дня остаётся открытым для записи.

  Разграничение проверяется в двух местах:
  - **Полностью закрытые дни** (`GET /api/closed-dates?masterId=...`,
    только строки без времени) — их клиентский календарь
    (`components/Calendar.js`) сразу показывает зачёркнутыми и
    недоступными для выбора.
  - **Частичное закрытие по времени** — учитывается в
    `GET /api/availability` (нужные слоты в сетке времени становятся
    недоступны, остальные — как обычно) и общая логика вынесена в
    `lib/closures.js` (`isWholeDayClosed`, `isTimeClosed`).

  `POST /api/appointments/create` отдельно перепроверяет оба случая на
  сервере — обойти закрытие через прямой запрос к API тоже нельзя.
