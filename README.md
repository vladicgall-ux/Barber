...
- То же самое подтверждение или отмену можно сделать **прямо из Telegram**,
  не открывая приложение: уведомление о новой записи, которое приходит в
  `TELEGRAM_ADMIN_CHAT_ID` (`lib/notify.js`), несёт две inline-кнопки —
  «✅ Подтвердить» и «❌ Отменить». Нажатие обрабатывается в
  `pages/api/bot/telegram-webhook.js` (`callback_query` с
  `data: "confirm:<id>"` или `"cancel:<id>"`), проверяет, что нажавший — id
  из `ADMIN_IDS` (`lib/auth/requireActiveUser.js#isAdminId`), и вызывает ту
