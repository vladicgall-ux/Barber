...
// Creates or updates a user by their Telegram, MAX, or VK id, refreshing
// their name (and, for VK, phone) from the platform on every login.
...
export async function upsertUserByVkId(vkId, { firstName, lastName, phone } = {}) {
  return upsertUser({ vk_id: vkId }, { firstName, lastName, phone });
}

async function upsertUser(identity, { firstName, lastName, phone }) {
  const column = Object.keys(identity)[0];
  ...
  if (existing) {
    ...
    if (phone && !existing.phone_confirmed) {
      patch.phone = phone;
