...
function bannerPhotoUrl() {
  const url = process.env.SITE_URL;
  return url ? `${url}/images/hero.png` : undefined;
}
...
export async function handleIncomingMessage({
  ...
  if (codeMatch) {
    // A 6-digit code is only 1 in a million — without a guess limit, an
    // attacker could just spam random codes hoping to hit someone else's
    // pending login. Cap guesses per sender instead of relying on the
    // 10-minute TTL alone.
    const { allowed } = await checkRateLimit(`code_guess:${platform}:${platformUserId}`, 10, 10 * 60 * 1000);
    if (!allowed) {
      await send('Слишком много попыток ввода кода. Подождите немного и попробуйте снова.');
      return;
    }
  ...
  await send(
    '💈 Привет! Я бот барбершопа BLACK BEARD.\n\n' +
      ...
      (platform === 'vk' ? appLinkLine(platform) : ''),
    { replyMarkup, photoUrl: bannerPhotoUrl() }
  );
}
