...
//
// `scopedMasterId` (from a master-scoped admin's admin_master_id) restricts
// the update to that master's own appointments — a global admin (null)
// can confirm any appointment. Without this, a master-scoped admin could
// confirm/cancel any other master's bookings by guessing an appointmentId,
// even though the UI/notifications only ever show them their own.
export async function confirmAppointment(appointmentId, scopedMasterId = null) {
  ...
  if (user && (user.telegram_id || user.max_id || user.vk_id)) {
    ...
    await notifyClient({ telegramId: user.telegram_id, maxId: user.max_id, vkId: user.vk_id, text }).catch(
      (e) => console.error('Client confirm-notify error', e)
    );
  }
